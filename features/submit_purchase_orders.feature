Feature: Submit Purchase Orders
  ...
  Scenario Outline: create purchase orders
    ...
    And the purchase order file name should be formatted YYMMDDHHMMSS.fbo
  ...
  Scenario Outline: submit purchase orders
    Given <order count> orders exist
    And each order has <line item count> line item with a quantity of <quantity count>
    And each order is completed
    When I create a purchase order
    And I send each purchase order to the server
    Then the server should contain
